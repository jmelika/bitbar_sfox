...
DELTA=$(curl -s "https://api.sfox.com/v1/markets/orderbook/ethbtc" | jq -r '[.asks[0] | .[0]], .bids[0] | .[0]')
...
case "$SPREAD_ARB" in
-*)
    echo "EB -$SPREAD_ARB | color=red"
    tweet.sh post ARB between ETH and BTC at -$SPREAD_ARB
    ;;
*)
    echo "EB $SPREAD_ARB"
    ;;
